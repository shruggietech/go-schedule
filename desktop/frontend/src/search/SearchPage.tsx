...
export function SearchPage({ bridge, onOpen }: { bridge: SearchBridge; onOpen(intent: Drilldown): Promise<void> }) {
  ...
  const open = (match: SearchMatch) => {
    ...
    void onOpen({ registrationKey: match.registrationKey, expectedDaemonId: match.expectedDaemonId, label: `${match.sourceLabel} (${match.sourceShortId})`, destination, taskId: match.result.task_id, recordId: match.result.object_id, context: `${match.result.kind}: ${match.result.name}` })
  }
  const execute = async () => {
  ...
  const grouped = useMemo(() => Object.entries(chosen.reduce<Record<string, SearchMatch[]>>((acc, match) => { (acc[`${match.sourceLabel} (${match.sourceShortId})`] ??= []).push(match); return acc }, {})), [chosen])
  return <>
