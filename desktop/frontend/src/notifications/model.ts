...
export type Delivery = { id: string; channelId?: string; channelName: string; destinationSummary: string; kind: 'test' | 'task_outcome' | 'daemon_health'; taskId?: string; taskName?: string; groupId?: string; groupName?: string; runId?: string; state: 'queued' | 'retrying' | 'sending' | 'successful' | 'failed'; attempts: number; nextAttemptAt?: string; createdAt: string; claimedAt?: string; completedAt?: string; lastStatus?: number; lastError?: string; conditionKind?: string; conditionSummary?: string }
