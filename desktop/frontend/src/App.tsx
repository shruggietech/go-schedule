...
export function App({ bridge = desktopBridge, tasks = nativeTaskBridge, automation = nativeAutomationBridge, operations = nativeOperationsBridge, notifications = nativeNotificationBridge, settings = nativeSettingsBridge, agentAccess = nativeAgentAccessBridge }: { bridge?: DesktopBridge; tasks?: TaskBridge; automation?: AutomationBridge; operations?: OperationsBridge; notifications?: NotificationBridge; settings?: SettingsBridge; agentAccess?: AgentAccessBridge }) {
  ...
  const [announcementFeedback, setAnnouncementFeedback] = useState<ShellFeedback>()
  ...
  useEffect(() => {
    ...
    setAnnouncementFeedback({ identity: `connection:${feedbackSequence.current}`, message: announcement, tone: 'success' })
  }, [announcement])
  useEffect(() => {
    if (!desktopSettings.status) return
    const next: ShellFeedback = { identity: `settings:${desktopSettings.status.id}`, message: desktopSettings.status.message, tone: desktopSettings.status.outcome === 'accepted' ? 'success' : 'error' }
    if (next.tone === 'error') setSettingsError(next)
    else { setSettingsError(undefined); setAnnouncementFeedback(next) }
