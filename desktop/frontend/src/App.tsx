...
export function App({ bridge = desktopBridge, tasks = nativeTaskBridge, automation = nativeAutomationBridge, operations = nativeOperationsBridge, notifications = nativeNotificationBridge, settings = nativeSettingsBridge, agentAccess = nativeAgentAccessBridge }: { bridge?: DesktopBridge; tasks?: TaskBridge; automation?: AutomationBridge; operations?: OperationsBridge; notifications?: NotificationBridge; settings?: SettingsBridge; agentAccess?: AgentAccessBridge }) {
  ...
  const [feedback, setFeedback] = useState<ShellFeedback>()
  ...
  useEffect(() => {
    ...
    setFeedback({ identity: `connection:${feedbackSequence.current}`, message: announcement, tone: 'success' })
  }, [announcement])
  useEffect(() => {
    if (!desktopSettings.status) return
    setFeedback({ identity: `settings:${desktopSettings.status.id}`, message: desktopSettings.status.message, tone: desktopSettings.status.outcome === 'accepted' ? 'success' : 'error' })
  ...
  return <Shell route={route} onRoute={setRoute} appearance={appearance} onAppearance={saveAppearance} appearancePending={desktopSettings.pending} connection={snapshot} feedback={feedback} onRetry={() => void retry()} onQuit={() => void bridge.quit()}>
