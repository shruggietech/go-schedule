...
const workspace: SettingsWorkspace = {
  preferences: {
    ...
    transition: {
      status: "migrated",
      retired: ["appearance.font", "appearance.scroll_sensitivity"],
    },
  },
  ...
  storage: [
    {
      id: "database",
      label: "Task database",
      path: "/var/lib/goschedule/tasks.db",
      owner: "go-schedule",
      scope: "machine",
      existence: "present",
      normalRemoval: "Preserved by normal software removal",
      explicitWipe: "No built-in data wipe on this platform",
      copyable: true,
    },
    {
      id: "logs",
      ...
      normalRemoval: "Preserved by normal software removal",
      explicitWipe:
        "Preserved because it is outside the application-owned data root",
      copyable: false,
    },
  ],
  product: {
    name: "go-schedule",
    version: "1.2.0",
    publisher: "ShruggieTech",
    links: [
      {
        key: "source",
        label: "Source repository",
        destination: "https://github.com/shruggietech/go-schedule",
      },
    ],
  },
...
describe("SettingsPage", () => {
  it("explains migration, retired controls, storage truth, and offline state", () => {
    renderPage();
    expect(
      screen.getByRole("heading", { name: "Appearance migrated" }),
    ).toBeVisible();
    expect(
      screen.getByText(/font selection and scroll sensitivity were retired/i),
    ).toBeVisible();
    ...
    expect(
      screen.getByText("Daemon storage details are unavailable"),
    ).toBeVisible();
  ...
  it("routes appearance, restore, copy, link, and recovery actions by identifiers", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Restore desktop defaults" }),
    );
    await user.click(screen.getByRole("button", { name: "Copy Task database path" }));
  ...
  it("shows a useful local failure without inventing content", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Desktop settings are unavailable" }),
    ).toBeVisible();
    expect(
