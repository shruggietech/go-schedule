...
export function ConnectionsPage({ snapshot, retryPending, onRetry }: { snapshot: ConnectionSnapshot; retryPending: boolean; onRetry(): void }) {
  ...
  return <><header className="page-header"><div><p className="eyebrow">This computer</p><h1>Connections</h1><p>Local scheduler diagnosis, recovery, and bounded remote enrollment.</p></div><StatusBadge state={snapshot.state} /></header><section className="panel connection-workspace" aria-busy={busy}><div><p className="eyebrow">Current diagnosis</p><h2>{snapshot.target.displayName}</h2><p>{snapshot.message}</p><p>{guidance[snapshot.state]}</p></div><dl><dt>Platform</dt><dd>{snapshot.target.platform}{snapshot.target.architecture ? `/${snapshot.target.architecture}` : ''}</dd><dt>Version</dt><dd>{snapshot.target.version || 'Unavailable'}</dd><dt>Capabilities</dt><dd>{snapshot.target.capabilities.length ? snapshot.target.capabilities.join(', ') : 'Unavailable'}</dd><dt>Last successful connection</dt><dd>{snapshot.lastSuccessfulAt || 'Not available in this session'}</dd></dl><Button disabled={busy || !snapshot.action} onClick={onRetry}>{busy ? 'Trying again' : snapshot.action ? 'Try again' : 'No retry needed'}</Button></section><PairingForm /></>
}
