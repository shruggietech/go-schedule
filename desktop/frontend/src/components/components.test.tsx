...
describe("shared component catalog", () => {
  ...
  it("provides status, notice, state, field, table, disclosure, and notification semantics", () => {
    render(
      <>
        ...
        <Notice title="Delayed">Updates are delayed.</Notice>
        <StatePanel title="No tasks" detail="Create one later." busy />
        <Field label="Name" help="Required" error="Enter a name">
          <input />
        </Field>
        <DataTable caption="Tasks" headings={["Name"]} rows={[[<>Backup</>]]} />
        <Disclosure summary="Details">More information</Disclosure>
        <ToastRegion message="Saved" />
      </>,
    );
  ...
  it("exposes compact semantic button variants and pending state", () => {
    render(
      <>
        <Button variant="affirmative">Enable</Button>
  ...
  it("provides responsive administration semantics and complete literal values", () => {
    render(
      <CardSection
        eyebrow="Storage"
        title="Task database"
        status={<StatusLabel tone="positive">Present</StatusLabel>}
      >
        <FormGrid>
          <Field label="Label">
            <input />
        ...
        <PathDisplay
          label="Task database path"
          value="C:\\ProgramData\\ShruggieTech\\go-schedule\\tasks.db"
        />
      </CardSection>,
    );
    expect(
      screen.getByRole("heading", { name: "Task database" }),
    ).toBeVisible();
    expect(screen.getByText("Present")).toHaveClass("status-label-positive");
    expect(screen.getByText("go-schedule").closest("dl")).toHaveClass(
      "description-list",
    );
    expect(screen.getByLabelText("Task database path")).toHaveTextContent(/ProgramData.*tasks\.db/);
    expect(screen.getByLabelText("Label").closest(".form-grid")).not.toBeNull();
  });

  it("supports a task-specific safe close label and restores focus when the dialog unmounts", () => {
    const ref = createRef<HTMLButtonElement>();
    const { rerender } = render(
      <>
        <button ref={ref}>Create task</button>
        <Dialog
          open
          title="Create task"
          closeLabel="Cancel"
  ...
  it("dismisses persistent notices explicitly", async () => {
    const user = userEvent.setup();
    render(
      <Notice title="Action needed" tone="error">
        Try again.
      </Notice>,
    );
    await user.click(
      screen.getByRole("button", { name: "Dismiss Action needed" }),
    );
  ...
  it("restores a dismissed JSX notice when its event identity changes", async () => {
    const user = userEvent.setup();
    const { rerender } = render(
      <Notice title="Could not save group" tone="error" identity="first">
        <span>Enter a group name.</span>
      </Notice>,
    );
    await user.click(
      screen.getByRole("button", { name: "Dismiss Could not save group" }),
    );
    rerender(
      <Notice title="Could not save group" tone="error" identity="second">
        <span>The group name is already in use.</span>
      </Notice>,
    );
  ...
  it("restarts identical transient feedback when its event identity changes", async () => {
    const user = userEvent.setup();
    const { rerender } = render(
      <ToastRegion message="Appearance saved." identity={1} />,
    );
    await user.click(
  ...
  it("preserves compound success content in transient feedback", () => {
    render(
      <Notice title="Ready" tone="success">
        <>
          <span>Preview generated.</span>
          <strong> Review the command.</strong>
        </>
      </Notice>,
    );
    expect(screen.getByRole("status")).toHaveTextContent(
      "Ready: Preview generated. Review the command.",
    );
  });
