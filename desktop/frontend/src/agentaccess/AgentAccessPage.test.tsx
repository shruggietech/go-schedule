...
const off: AgentAccessWorkspace = { stdioDescription: 'Available on demand. Stdio opens no network listener.', http: { enabled: false, allowedOrigins: [], requestCount: 0 }, authorities: [{ name: 'Observe', status: 'available', description: 'Read bounded data.' }, { name: 'Operate', status: 'future', description: 'Unavailable.' }, { name: 'Manage', status: 'future', description: 'Unavailable.' }] }
const bridge = (workspace = off): AgentAccessBridge => ({ workspace: vi.fn().mockResolvedValue({ action: 'load_agent_access', outcome: 'accepted', message: '', workspace }), enable: vi.fn().mockResolvedValue({ action: 'enable_agent_access', outcome: 'accepted', message: 'copied', workspace }), rotate: vi.fn(), revoke: vi.fn(), openGuide: vi.fn() })
...
describe('Agent Access page', () => {
  ...
  it('shows active evidence and lifecycle controls', async () => {
    const active = { ...off, http: { enabled: true, endpoint: 'http://127.0.0.1:43123/mcp', allowedOrigins: [], credentialFingerprint: 'abc', enabledAt: '2026-09-09T12:00:00Z', clientName: 'Codex', lastAccessedAt: '2026-09-09T12:01:00Z', requestCount: 2 } }
    render(<AgentAccessPage bridge={bridge(active)} available refreshToken={1} />)
