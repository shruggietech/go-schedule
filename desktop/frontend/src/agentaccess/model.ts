export interface AgentHTTPStatus { enabled: boolean; endpoint?: string; allowedOrigins: string[]; credentialFingerprint?: string; enabledAt?: string; clientName?: string; permission?: string; lastAccessedAt?: string; requestCount: number }
...
export interface AgentAccessDraft { clientName: string; port: number; allowedOrigins: string[]; permission: 'observe' | 'operate' }
