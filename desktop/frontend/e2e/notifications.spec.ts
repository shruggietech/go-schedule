...
test('manages notification setup and large redacted history accessibly', async ({ page }) => {
  ...
  await expect(page.getByText('1 active destination', { exact: true })).toBeVisible(); await expect(page.getByText('1 configured task', { exact: true })).toBeVisible(); await expect(page.getByTestId('recent-notification-result')).toHaveCount(5)
