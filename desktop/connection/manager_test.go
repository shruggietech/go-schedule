...
func TestManagerExactRetryCadence(t *testing.T) {
	...
	wants := []time.Duration{500 * time.Millisecond, 2 * time.Second, 10 * time.Second}
...
func TestManagerDegradesAndRecoversAfterEventFailure(t *testing.T) {
	...
	if got := <-scheduler.delays; got != 500*time.Millisecond {
