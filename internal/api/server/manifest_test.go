...
func TestManifestReportsBoundedDeterministicDaemonFacts(t *testing.T) {
	...
	if !sort.StringsAreSorted(first.Capabilities) || len(first.Capabilities) != 13 || !contains(first.Capabilities, "actor-authorization") || !contains(first.Capabilities, "bundles") || !contains(first.Capabilities, "management-audit") || !contains(first.Capabilities, "search") || first.Platform.OS == "" || first.Platform.Architecture == "" {
...
func TestManifestReportsRemoteMCPWithoutExposingConfiguration(t *testing.T) {
	...
	if !contains(manifest.Capabilities, "remote-mcp") || len(manifest.Capabilities) != 14 || !sort.StringsAreSorted(manifest.Capabilities) {
