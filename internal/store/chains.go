...
func (s *Store) RecordRunAndCreateDeliveries(r *domain.Run, incomingDeliveryID string) error {
	...
	defer func() { _ = tx.Rollback() }()
	_, err = tx.Exec(`INSERT INTO runs(id,task_id,scheduled_for,started_at,ended_at,outcome,exit_code,output,trigger,source_task_id,source_run_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
		r.ID, r.TaskID, fmtTime(r.ScheduledFor), fmtTimePtr(r.StartedAt), fmtTimePtr(r.EndedAt), string(r.Outcome), nullInt(r.ExitCode), r.Output, string(r.Trigger), nullStr(r.SourceTaskID), nullStr(r.SourceRunID))
	if err != nil {
