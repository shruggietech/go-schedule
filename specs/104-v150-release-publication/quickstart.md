...
6. Wait for waived promotion to pass, then download public assets afresh and verify `SHA256SUMS.txt` and latest-release identity. If promotion stops after checksum upload, rerun the same workflow with the same tag: it accepts only an existing checksum file that exactly matches a fresh calculation from the eight staged assets.
