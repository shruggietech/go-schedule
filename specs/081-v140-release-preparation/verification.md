...
The one authorized second Codex review identified three remaining trust gaps. Current and legacy desktop, window, and setup rules are now selected from the validated candidate semantic version rather than caller-controlled metric presence, and current v1.4 native evidence requires schema version two. The upgrade observation independently requires retained tasks, run history, appearance intent, daemon identity, service operation, and local access. Release preflight now requires one exact whole-line Markdown changelog destination, rejecting suffixed URLs and plain-text mentions. Focused release-gate, integration, automation-mutation, formatting, PowerShell syntax, and diff-integrity checks passed after these corrections, followed by all eight canonical gates. No further automated review round is permitted.
