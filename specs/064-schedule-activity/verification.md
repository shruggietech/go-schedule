...
| `npm test -- --run` from `desktop/frontend/` | PASS, 15 files and 44 tests |
...
| race | PASS |
| gui | PASS |
| coverage | PASS, engine 81.9%, schedule 89.2%, timezone 91.3%, store 80.1%, catchup 88.9%, logbus 91.1% |
...
- Activity retains persisted runs, recent daemon logs, alerts, exact log-path metadata, text and typed filters, detailed output and provenance, individual acknowledgement, and non-destructive Clear View behavior.
...
- No daemon API, persistence, scheduling, retention, or packaging input changed.
