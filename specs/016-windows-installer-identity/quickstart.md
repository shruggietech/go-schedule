...
Expected: both commands pass. The Go test also proves independent failures for
missing or misdirected icon relationships.

## Candidate MSI table contract

Build the MSI with WiX 5.0.2 using the same inputs as the release workflow, then
run:
...
reported as `proven`. This does not install the package.

## Clean Windows lifecycle

Copy the MSI and repository `test/windows/` directory to a disposable Windows
...
All eight gates must pass before the local feature commit. Unavailable native or
published evidence remains visible in `verification.md`; it is not converted to
a pass.
