...
This feature introduces no persisted application data. Its validation model has two static concepts.

## Token role

- **Safe base**: default foreground for every classified or unclassified code token.
...
Every token first receives Safe base and may then receive exactly one named foreground role. Styling-only roles such as bold and italic do not change the foreground contract.
...
An opening fence without one of these values is invalid. Closing fences carry no category.
