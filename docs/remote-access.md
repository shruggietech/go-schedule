...
The desktop All Systems workspace observes This computer and every saved profile without changing the selected target. It makes one bounded Observe-authorized summary request per daemon with at most four targets in flight and a five-second deadline for each target. Successful results from the current desktop session may remain visible as stale after a later target-specific failure. No overview cache survives restart. Opening a task, schedule, activity record, alert, or notification problem selects the exact profile ID before navigation, so duplicate display names never determine authority or target ownership.

The desktop Search workspace independently queries This computer and every saved profile without changing the selected target. It allows at most eight searches in flight, gives each target three seconds, publishes progressive source-qualified results, and keeps target failures separate from successful matches. The remote allowlist exposes the same Observe-authorized `GET /api/v1/search` contract as local IPC while retaining the response redaction and collection bounds defined in the OpenAPI source.

Opening a search result selects the immutable registration key and then confirms that the connected daemon still has the expected installation identity. A mismatch fails closed and leaves Search intact. Acknowledge, enable, disable, and run-now are deliberate Operate-authorized actions. The desktop groups selected objects by daemon for confirmation, reconstructs each exact target from current profile and native credential storage, revalidates health, identity, authority, object existence, and compatible state, and then calls the existing audited endpoint. Targets execute independently, successful actions are never rolled back, and an uncertain remote response is preserved without automatic replay.
