...
function Tasks({ snapshot, condition, onEdit, onCancel, onRetry }: { snapshot: ProofSnapshot; condition: Condition; onEdit(): void; onCancel(): void; onRetry(): void }) {
  ...
  if (condition === 'degraded' && snapshot.tasks.length === 0) return <StatePanel title="Tasks unavailable" detail={snapshot.health.message} action="Try again" onAction={onRetry} />
  ...
  const selectedTask = snapshot.tasks[0]
  if (!selectedTask) return <StatePanel title="No tasks yet" detail="Create a task to run recurring work on this computer." action="Create task" onAction={onEdit} />
  const resultCondition: Condition = selectedTask.lastResult === 'failed' ? 'degraded' : selectedTask.lastResult === 'success' ? 'success' : 'empty'
  return <div className="split"><section aria-labelledby="task-list-title" className="panel list-panel"><div className="panel-heading"><div><p className="eyebrow">{snapshot.tasks.length} {snapshot.tasks.length === 1 ? 'task' : 'tasks'}</p><h2 id="task-list-title">Scheduled work</h2></div><button className="quiet-button">Filter</button></div><div className="task-list">{snapshot.tasks.map((task, index) => <button className={`task-row ${index === 0 ? 'selected' : ''}`} key={task.id} onClick={onEdit}><span><strong>{task.name}</strong><small>{task.schedule}</small></span><span><Status condition={task.lastResult === 'failed' ? 'degraded' : task.enabled ? 'connected' : 'empty'} /><small>{task.nextRun || 'Paused'}</small></span></button>)}</div></section><aside aria-labelledby="task-detail-title" className="panel inspector"><p className="eyebrow">Selected task</p><h2 id="task-detail-title">{selectedTask.name}</h2><dl><div><dt>Schedule</dt><dd>{selectedTask.schedule}</dd></div><div><dt>Next run</dt><dd>{selectedTask.nextRun || 'Paused'}</dd></div><div><dt>Last result</dt><dd><Status condition={resultCondition} /></dd></div></dl><button className="secondary-button" onClick={onEdit}>Edit task</button></aside></div>
}

function Editor({ condition, onCancel, onSave }: { condition: Condition; onCancel(): void; onSave(): void }) {
  const errorId = useId()
  return <section className="panel form-panel" aria-labelledby="editor-title"><div className="panel-heading"><div><p className="eyebrow">Runs on This computer</p><h2 id="editor-title">Task details</h2></div>{condition === 'success' && <Status condition="success" />}</div><form onSubmit={(event) => { event.preventDefault(); onSave() }}><label>Task name<input defaultValue={condition === 'new' ? '' : 'Nightly backup'} aria-invalid={condition === 'validation-error'} aria-describedby={condition === 'validation-error' ? errorId : undefined} /></label>{condition === 'validation-error' && <p className="field-error" id={errorId}>Enter a task name before saving.</p>}<label>Command<input className="mono" defaultValue="gosched backup --all" /></label><div className="form-grid"><label>Schedule<input defaultValue="Every day at 23:00" /></label><label>Time zone<select defaultValue="local"><option value="local">Local time</option><option value="utc">UTC</option></select></label></div><label className="check"><input type="checkbox" defaultChecked />Enable this task after saving</label><div className="actions"><button type="button" className="quiet-button" onClick={onCancel}>Cancel</button><button type="submit" className="primary-button">Save task</button></div></form></section>
}
...
export function App({ snapshotLoader = loadSnapshot }: { snapshotLoader?: typeof loadSnapshot }) {
  ...
  const targetDialogInvokerRef = useRef<HTMLButtonElement | null>(null)
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        ...
        targetDialogInvokerRef.current?.focus()
      }
    }
  ...
  const openTargetDialog = (invoker: HTMLButtonElement) => {
    targetDialogInvokerRef.current = invoker
  ...
  const runPrimaryAction = (event: React.MouseEvent<HTMLButtonElement>) => {
    if (page === 'tasks') selectPage('editor')
    else if (page === 'targets') openTargetDialog(event.currentTarget)
  ...
  return <div className="app" data-appearance={appearance}>
    ...
    <div className="workspace"><header className="target-bar" aria-label="Target context"><div><span className="target-dot" aria-hidden="true" /><strong>{snapshot.target.displayName}</strong><span>{snapshot.target.platform}</span></div><Status condition={snapshot.target.connection as Condition} /><button ref={targetButtonRef} onClick={(event) => openTargetDialog(event.currentTarget)}>Switch target</button></header>
      <main id="main-content" ref={mainRef} tabIndex={-1}><header className="page-header"><div><p className="eyebrow">This computer</p><h1>{title}</h1><p>{page === 'tasks' ? 'Review, create, and maintain scheduled work.' : page === 'editor' ? 'Define what runs and when it runs.' : page === 'schedule' ? 'See when enabled work will run next.' : page === 'activity' ? 'Understand recent results and investigate failures.' : 'Keep execution context visible and deliberate.'}</p></div>{page !== 'editor' && <button className="primary-button" onClick={runPrimaryAction}>{primary}</button>}</header>
        ...
        {page === 'editor' && <Editor condition={condition} onCancel={() => selectPage('tasks')} onSave={() => setCondition('success')} />}
        {page === 'schedule' && <ScheduleView />}
        {page === 'activity' && <Activity condition={condition} />}
        {page === 'targets' && <Targets snapshot={snapshot} />}
      </main>
    </div>
    {targetDialogOpen && <div className="dialog-backdrop"><section aria-labelledby="switch-target-title" aria-modal="true" className="dialog" role="dialog"><div className="panel-heading"><div><p className="eyebrow">Execution context</p><h2 id="switch-target-title">Switch target</h2></div><button ref={dialogCloseRef} className="quiet-button" onClick={closeTargetDialog}>Close</button></div><p>Choose where subsequent task changes will run.</p><button className="target-card selected"><span className="target-mark" aria-hidden="true">L</span><span><strong>This computer</strong><small>Local scheduler service</small></span><Status condition="connected" /><span className="selection-label">Current target</span></button></section></div>}
